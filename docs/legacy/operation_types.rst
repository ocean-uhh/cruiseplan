...
1. **Point Operations**: Operations at fixed geographic locations
2. **Line Operations**: Operations along defined routes  
...
Point operations represent activities conducted at fixed geographic locations. They are defined in the ``points`` catalog and converted to ``PointOperation`` objects during runtime processing.
...
Line operations represent movement or data collection along defined routes. They are defined in the ``lines`` catalog and converted to ``LineOperation`` objects during runtime processing.
...
.. list-table:: Line Operation Types
   ...
   * - ``CTD``
     - ``section``
     - Scientific transit with multiple CTD points that can be expanded into individual points

**Navigation-Only Transits**:
  When no ``operation_type`` is specified, lines represent pure vessel movement with no scientific activities.

**Section Expansion**:
  Special transit types like CTD sections can be automatically expanded into individual points using the ``cruiseplan enrich --expand-sections`` command:

  - **Interpolation**: Creates individual points along the transit route
  - **Naming Convention**: Generated points use a sequential naming pattern (e.g., "Section_001", "Section_002")
  ...
  **Automatic Routing Updates**:
    ...
    - ``activities`` expanded to include all generated points
...
**Speed Override**:
  Individual lines can specify ``vessel_speed`` to override the default cruise speed.
...
Area operations represent activities conducted over defined geographic regions. They are defined in the ``areas`` catalog and converted to ``AreaOperation`` objects during runtime processing.
...
.. code-block:: yaml
   ...
   areas:
     - name: "Multibeam_Grid_A"
       ...
       corners:
         - latitude: 50.0000
           longitude: -40.0000  # Southwest corner - High precision
         - latitude: 51.0000
           longitude: -40.0000  # Southeast corner  
         - latitude: 51.0000
           longitude: -39.0000  # Northeast corner
         - latitude: 50.0000
           longitude: -39.0000  # Northwest corner
     ...
     - name: "Triangular_Survey_Zone"
       ...
       corners:
         - latitude: 52.5
           longitude: -41.2   # Point A - Standard precision
         - latitude: 53.0
           longitude: -40.8   # Point B
         - "52.2, -40.5"   # Point C
...
- **Seamless integration**: Areas work with points and lines in unified workflows
