...
- ``cruiseplan.runtime.cruise``: Main CruiseInstance class and configuration loading
- ``cruiseplan.timeline``: Distance, duration, and scheduling calculations
...
.. code-block:: python
    ...
    print(f"Number of points: {len(cruise.point_registry)}")
...
.. code-block:: python
    ...
    cruise = CruiseInstance.from_yaml("cruise.yaml")
...
.. code-block:: python
    ...
    cruise_config = {
        ...
        'first_station': points[0].name,
        'last_station': points[-1].name,
        'points': [station.dict() for station in points],
        'legs': [{
            'name': 'North_Atlantic_Transect',
            'strategy': 'sequential',
            'activities': [s.name for s in points]
        }]
    }
...
.. code-block:: python
    ...
    pangaea_manager = PangaeaManager()
