...
.. list-table:: Command Flow Summary
   ...
   * - ``enrich``
     ...
     - `*.yaml`
   * - ``validate``
     - Check configuration (alternative)
     - YAML file
     - Validation report
     - Text output     
   * - ``map``
     - Visualize cruise plan (alternative)
     - YAML file
     - Map figure
     - `*.png`, `*.pdf`
   * - ``schedule``
...
For complex cruises with multiple operational phases between different ports, these must be separated into distinct "legs". Each leg requires its own departure and arrival ports and can have its own activities and strategy. This enables proper routing calculations and port logistics.
...
.. code-block:: yaml
   ...
   legs:
     - name: "Northern_Survey"
       ...
       arrival_port: "port_nuuk"
       first_waypoint: "STN_001"
       last_waypoint: "STN_003"
       strategy: "sequential"
       activities: ["STN_001", "STN_002", "STN_003"]
       
     - name: "Mooring_Operations"
       departure_port: "port_nuuk"
       arrival_port: "port_reykjavik"
       first_waypoint: "MOOR_A_RECOVERY"
       last_waypoint: "MOOR_B_DEPLOYMENT"
       strategy: "sequential"  
       activities: ["MOOR_A_RECOVERY", "MOOR_B_DEPLOYMENT"]
       
     - name: "Southern_Return"
       departure_port: "port_reykjavik" 
       arrival_port: "port_cadiz"
       first_waypoint: "STN_004"
       last_waypoint: "STN_005"
       strategy: "sequential"
       activities: ["STN_004", "STN_005"]

**For organizing activities within a single leg**, use clusters instead:
...
.. code-block:: yaml
   ...
   legs:
     - name: "Main_Survey"
       departure_port: "port_halifax"
       arrival_port: "port_halifax"
       first_waypoint: "CTD_01"
       last_waypoint: "CTD_03"
