...
.. note::
  See :ref:`Bathymetry subcommand CLI reference <subcommand-bathymetry>` in :doc:`../cli/bathymetry` for full options and examples.
...
- ``{search_name}_stations.pkl``: Station database for interactive picker
...
- Creates a pickled database file: ``campaigns_stations.pkl``

**Output files:**

- ``campaigns_stations.pkl``: Station database for interactive picker
...
.. code-block:: bash
   ...
   cruiseplan stations -p pangaea_data/campaigns_stations.pkl --lat 55 70 --lon -60 -20 -o output_dir/
