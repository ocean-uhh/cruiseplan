...
class TestDemoMain:
    ...
    def test_main_function_complete_flow(
        self, mock_print, mock_bathymetry, mock_pangaea_class, mock_station_picker_class
    ):
    ...
    @patch("cruiseplan.cli.demo.bathymetry")
    @patch("builtins.print")
    def test_main_with_failed_pangaea_fetch(
        self, mock_print, mock_bathymetry, mock_pangaea_class, mock_station_picker_class
    ):
        ...
        mock_station_picker_class.assert_called_once_with(
            campaign_data=expected_dummy_data, output_file="demo_stations.yaml"
        )

        # Check that failure message was printed
        print_calls = [call[0][0] for call in mock_print.call_args_list]
        assert any(
            "Real fetch failed (offline?), using Dummy Data." in call
    ...
    @patch("cruiseplan.cli.demo.bathymetry")
    @patch("builtins.print")
    def test_main_with_successful_pangaea_fetch(
        self, mock_print, mock_bathymetry, mock_pangaea_class, mock_station_picker_class
    ):
        ...
        test_datasets = [
            {
                ...
                "latitude": [50],
                "longitude": [-45],
                "doi": "10.1594/PANGAEA.1",
            },
            {
                "label": "Campaign2",
                "latitude": [51],
                "longitude": [-44],
                "doi": "10.1594/PANGAEA.2",
            },
            {
                "label": "Campaign3",
                "latitude": [52],
                "longitude": [-43],
                "doi": "10.1594/PANGAEA.3",
            },
        ]
        ...
        mock_station_picker_class.assert_called_once_with(
            campaign_data=test_datasets, output_file="demo_stations.yaml"
        )
    ...
    @patch("cruiseplan.cli.demo.PangaeaManager")
    @patch("cruiseplan.cli.demo.bathymetry")
    def test_main_bathymetry_depth_values(
        self, mock_bathymetry, mock_pangaea_class, mock_station_picker_class
    ):
