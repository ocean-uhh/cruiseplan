...
class TestCruise:
    ...
    def test_cruise_initialization_success(self, mock_load_yaml):
        ...
        with patch('cruiseplan.core.cruise.resolve_port_reference') as mock_resolve:
            ...
            assert len(cruise.station_registry) == 3
            assert "STN_001" in cruise.station_registry
            assert "STN_002" in cruise.station_registry
            assert "STN_003" in cruise.station_registry
            
            assert len(cruise.transit_registry) == 1
            assert "TRANS_001" in cruise.transit_registry
    ...
    def test_cruise_with_minimal_config(self, mock_load_yaml):
        ...
        with patch('cruiseplan.core.cruise.resolve_port_reference'):
            ...
            assert len(cruise.transit_registry) == 0
            assert len(cruise.area_registry) == 0
            assert len(cruise.port_registry) == 0
    ...
    def test_cruise_empty_registries(self, mock_load_yaml):
        ...
        with patch('cruiseplan.core.cruise.resolve_port_reference'):
            ...
            assert len(cruise.station_registry) == 0
            assert len(cruise.transit_registry) == 0
            assert len(cruise.area_registry) == 0
            assert len(cruise.port_registry) == 0
    ...
    def test_resolve_references_success(self, mock_load_yaml):
        ...
        with patch('cruiseplan.core.cruise.resolve_port_reference'):
            ...
            assert hasattr(leg.stations[0], 'name')
            assert hasattr(leg.stations[1], 'name')
    ...
    def test_resolve_references_invalid_station(self, mock_load_yaml):
        ...
        invalid_config["legs"] = [
            {
                "name": "Bad_Leg",
                "stations": ["STN_001", "INVALID_STATION"]
            }
        ]
    ...
    def test_resolve_config_ports_success(self, mock_load_yaml):
        """Test successful port resolution."""
        config_with_ports = self.test_yaml_data.copy()
        config_with_ports.update({
            "departure_port": "REYKJAVIK",
            "arrival_port": "REYKJAVIK"
        })
        ...
        with patch('cruiseplan.core.cruise.resolve_port_reference') as mock_resolve:
            ...
            # Should have called resolve_port_reference for both ports
            assert mock_resolve.call_count == 2

    @patch('cruiseplan.core.cruise.load_yaml')
    def test_resolve_config_ports_global_lookup(self, mock_load_yaml):
        """Test port resolution with global port lookup."""
        config_with_global_port = self.test_yaml_data.copy()
        config_with_global_port["departure_port"] = "UNKNOWN_PORT"
        mock_load_yaml.return_value = config_with_global_port
        
        # Mock global port definition
        mock_global_port = MagicMock()
        mock_global_port.name = "UNKNOWN_PORT"
        mock_global_port.latitude = 65.0
        mock_global_port.longitude = -18.0
        ...
        with patch('cruiseplan.core.cruise.resolve_port_reference') as mock_resolve:
            ...
            assert "UNKNOWN_PORT" in cruise.port_registry

    @patch('cruiseplan.core.cruise.load_yaml')
    def test_resolve_config_ports_not_found(self, mock_load_yaml):
        """Test port resolution when port is not found."""
        config_with_missing_port = self.test_yaml_data.copy()
        config_with_missing_port["departure_port"] = "MISSING_PORT"
    ...
    def test_resolve_list_with_objects(self, mock_load_yaml):
        ...
        with patch('cruiseplan.core.cruise.resolve_port_reference'):
            ...
            station_obj = cruise.station_registry["STN_001"]
            mixed_list = ["STN_002", station_obj]  # String and object
            
            resolved = cruise._resolve_list(mixed_list, cruise.station_registry, "Station")
    ...
    def test_resolve_list_invalid_reference(self, mock_load_yaml):
        ...
        with patch('cruiseplan.core.cruise.resolve_port_reference'):
            ...
            with pytest.raises(ReferenceError):
                cruise._resolve_list(invalid_list, cruise.station_registry, "Station")

    @patch('cruiseplan.core.cruise.load_yaml')
    def test_cluster_legacy_stations_field(self, mock_load_yaml):
        """Test handling of deprecated 'stations' field in clusters."""
        config_with_legacy = self.test_yaml_data.copy()
        config_with_legacy["legs"] = [
            {
                "name": "Legacy_Leg",
                "clusters": [
                    {
                        "name": "Legacy_Cluster",
                        "stations": ["STN_001", "STN_002"]  # Deprecated field
                    }
                ]
            }
        ]
        mock_load_yaml.return_value = config_with_legacy
        
        with patch('cruiseplan.core.cruise.resolve_port_reference'):
            cruise = Cruise("config_with_legacy.yaml")
