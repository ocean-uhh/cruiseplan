...
class TestTC2TwoLegsIntegration:
    ...
    def test_cruise_object_creation_and_port_resolution(self, base_config_path):
        ...
        assert hasattr(leg_atlantic.departure_port, 'latitude'), "Departure port should be resolved"
        assert hasattr(leg_atlantic.arrival_port, 'latitude'), "Arrival port should be resolved"
        ...
        assert hasattr(leg_north.departure_port, 'latitude'), "Departure port should be resolved"
        assert hasattr(leg_north.arrival_port, 'latitude'), "Arrival port should be resolved"
    ...
    def test_timeline_generation_with_expected_structure(self, base_config_path):
        ...
        activities = [(act["activity"], act.get("label", ""), act.get("leg_name", "")) for act in timeline]
    ...
    def test_specific_transit_distances(self, base_config_path):
        ...
        transit_activities = [act for act in timeline if act.get("transit_dist_nm", 0) > 0]
        
        # Should have 4 transit activities (Port_Departure and Port_Arrival for each leg)
        assert len(transit_activities) >= 4, "Expected at least 4 activities with transit distances"

        # Test specific expected distances (within 1% tolerance)
        halifax_to_stn001_activity = next(
            act for act in timeline 
            if act.get("activity") == "Port_Departure" and "Halifax" in act.get("label", "")
        )
        
        # Halifax to STN_001: 637.7 nm
        expected_distance_1 = 637.7
        actual_distance_1 = halifax_to_stn001_activity.get("transit_dist_nm", 0)
        assert abs(actual_distance_1 - expected_distance_1) / expected_distance_1 < 0.01, \
        ...
        stn001_to_bremerhaven_activity = next(
            act for act in timeline
            if act.get("activity") == "Port_Arrival" and "Bremerhaven" in act.get("label", "")
        )
        
        expected_distance_2 = 2124.8
        actual_distance_2 = stn001_to_bremerhaven_activity.get("transit_dist_nm", 0)
        assert abs(actual_distance_2 - expected_distance_2) / expected_distance_2 < 0.01, \
        ...
        bremerhaven_to_stn002_activity = next(
            act for act in timeline
            if act.get("activity") == "Port_Departure" and "Bremerhaven" in act.get("label", "") 
            and act.get("leg_name") == "Leg_North"
        )
        
        stn002_to_reykjavik_activity = next(
            act for act in timeline
            if act.get("activity") == "Port_Arrival" and "Reykjavik" in act.get("label", "")
        )

        # These should be > 0 and reasonable (rough bounds check)
        assert bremerhaven_to_stn002_activity.get("transit_dist_nm", 0) > 1000, "Bremerhaven to STN_002 should be substantial distance"
        assert stn002_to_reykjavik_activity.get("transit_dist_nm", 0) > 200, "STN_002 to Reykjavik should be reasonable distance"
    ...
    def test_html_output_leg_durations(self, base_config_path, temp_dir):
        ...
        assert "11.5 days" in html_content, "Leg_Atlantic should show 11.5 days total duration"
        assert "48.5 days" in html_content, "Leg_North should show 48.5 days total duration"
        ...
        assert "Halifax to Operations" in html_content, "Should show Halifax departure with ' to ' format"
        assert "Operations to Bremerhaven" in html_content, "Should show Bremerhaven arrival with ' to ' format"
        assert "Bremerhaven to Operations" in html_content, "Should show Bremerhaven departure with ' to ' format"
        assert "Operations to Reykjavik" in html_content, "Should show Reykjavik arrival with ' to ' format"
    ...
    def test_netcdf_output_mooring_duration(self, base_config_path, temp_dir):
        ...
        netcdf_generator.generate_point_operations(cruise.config, timeline, point_ops_path)
        ...
        operation_type_strings = [op.decode('utf-8') if isinstance(op, bytes) else str(op) for op in operation_types]
        
        # Find mooring indices
        mooring_indices = [i for i, op_type in enumerate(operation_type_strings) if 'mooring' in op_type.lower()]
        
        assert len(mooring_indices) > 0, "Should have at least one mooring operation in NetCDF"
        
        # Check that mooring duration matches DEFAULT_MOORING_DURATION_MIN (converted to hours)
        expected_duration_hours = DEFAULT_MOORING_DURATION_MIN / 60.0  # Convert minutes to hours
        for idx in mooring_indices:
            ...
            elif isinstance(mooring_duration, (int, float)) and mooring_duration > 10000:
                # Likely nanoseconds, convert to hours
                mooring_duration_hours = float(mooring_duration) / (1e9 * 3600.0)  # ns to hours
            else:
    ...
    def test_complete_two_leg_workflow(self, base_config_path, temp_dir):
        ...
        netcdf_generator.generate_point_operations(cruise.config, timeline, point_ops_path)
        netcdf_generator.generate_ship_schedule(timeline, cruise.config, ship_schedule_path)
        ...
        for format_name, file_path in outputs.items():
            ...
            assert file_path.stat().st_size > 0, f"{format_name} output should not be empty"
        ...
        assert total_duration > 1400 * 60, "Two-leg cruise should take more than 1400 hours total"  # minutes
        assert total_distance > 4000, "Two-leg cruise should cover more than 4000 nm total"
        
        # Validate mooring duration is substantial part of total
        mooring_duration = sum(
            act.get("duration_minutes", 0) for act in timeline 
            if act.get("activity") == "Mooring"
        )
        assert mooring_duration >= DEFAULT_MOORING_DURATION_MIN, "Should include full mooring duration"
        
        # Final check: ensure leg separation is maintained
        leg_atlantic_activities = [act for act in timeline if act.get("leg_name") == "Leg_Atlantic"]
        leg_north_activities = [act for act in timeline if act.get("leg_name") == "Leg_North"]
        
        assert len(leg_atlantic_activities) == 3, "Leg_Atlantic should have exactly 3 activities"
        assert len(leg_north_activities) == 3, "Leg_North should have exactly 3 activities"