...
def test_pangaea_merge_campaigns():
    ...
    datasets_unmerged = manager.fetch_datasets(dois, rate_limit=0.5, merge_campaigns=False)
    ...
    assert len(datasets_unmerged) == 4, f"Expected 4 unmerged datasets, got {len(datasets_unmerged)}"
    
    # Merged should have 3 datasets (DY081 entries merged)
    assert len(datasets_merged) == 3, f"Expected 3 merged datasets, got {len(datasets_merged)}"
    
    # Find the merged DY081 dataset
    dy081_datasets = [ds for ds in datasets_merged if ds.get('label') == 'DY081']
    assert len(dy081_datasets) == 1, f"Expected 1 merged DY081 dataset, got {len(dy081_datasets)}"
    ...
    dy081_dois = [doi for doi in dois if 'DY081' in doi or doi in ['10.1594/PANGAEA.935444', '10.1594/PANGAEA.935448']]
    assert len(dois_in_merged) == 2, f"Expected 2 DOIs in merged DY081, got {len(dois_in_merged)}"
    assert set(dy081_dois).issubset(set(dois_in_merged)), "Merged dataset should contain both DY081 DOIs"
