...
class TestTC3ClustersIntegration:
    ...
    def test_tc3_mooring_operations_count(self, tc3_config):
        ...
        mooring_ops = [
            ...
            if activity.get("activity") == "Mooring"
        ]
        
        # Expected: 12 mooring operations total (2 per leg × 6 legs)
        # Each leg has STN_003 and STN_004 with mooring operations
        assert len(mooring_ops) == 12, f"Expected 12 mooring operations, got {len(mooring_ops)}"
        
        # Verify total mooring duration is 12 hours (1 hour per operation)
        total_mooring_hours = sum(op.get("duration_minutes", 0) / 60.0 for op in mooring_ops)
        assert total_mooring_hours == 12.0, f"Expected 12 hours of mooring, got {total_mooring_hours}"
    ...
    def test_tc3_ctd_stations_count(self, tc3_config):
        ...
        ctd_ops = [
            activity for activity in timeline 
            if activity.get("activity") == "Station" and 
            activity.get("action") == "profile"
        ]
    ...
    def test_tc3_vessel_speed_differences(self, tc3_config):
        ...
        leg_faster = next(leg for leg in tc3_config.legs if leg.name == "Leg_Survey_Faster")
        
        # Leg_Survey should use default speed (None), Leg_Survey_Faster should have 12.0
        assert leg_survey.vessel_speed is None, "Leg_Survey should use default vessel speed"
        assert leg_faster.vessel_speed == 12.0, "Leg_Survey_Faster should have vessel_speed 12.0"
        ...
        leg_survey_transits = [
            activity for activity in timeline 
            if activity.get("activity") == "Transit" and activity.get("leg_name") == "Leg_Survey"
        ]
        leg_faster_transits = [
            activity for activity in timeline 
            if activity.get("activity") == "Transit" and activity.get("leg_name") == "Leg_Survey_Faster"
        ]
        
        # Verify transit speeds are applied correctly
        for transit in leg_survey_transits:
            assert transit.get("vessel_speed_kt") == 10.0, "Leg_Survey should use default 10.0 kt speed"
        
        for transit in leg_faster_transits:
            assert transit.get("vessel_speed_kt") == 12.0, "Leg_Survey_Faster should use 12.0 kt speed"
        
        # Calculate total leg durations and verify speed difference
        leg_survey_activities = [a for a in timeline if a.get("leg_name") == "Leg_Survey"]
        leg_faster_activities = [a for a in timeline if a.get("leg_name") == "Leg_Survey_Faster"]
        ...
        assert 20.0 < time_difference < 23.0, f"Expected ~21.3h difference, got {time_difference:.1f}h"
    ...
    def test_tc3_duplicate_station_warnings(self, tc3_config):
        ...
        duplicate4_leg = next(
            leg for leg in tc3_config.legs 
            if leg.name == "Leg_Survey_Duplicate4"
        )
        
        assert duplicate4_leg.first_station == "STN_001"
        
        # Find cluster containing first_station
        ctd_cluster5 = next(
            cluster for cluster in duplicate4_leg.clusters
    ...
    def test_tc3_reorder_leg_behavior(self, tc3_config):
        ...
        reorder_leg = next(
            leg for leg in tc3_config.legs 
            if leg.name == "Leg_Survey_Reorder"
        )
        
        # This leg should have STN_004 as first and STN_001 as last (reversed)
        assert reorder_leg.first_station == "STN_004"
        assert reorder_leg.last_station == "STN_001"
    ...
    def test_tc3_html_working_area_transits(self, tc3_config):
        ...
        expected_hours = 695.0  # Sum of all departure and arrival transits for 6 legs
        expected_distance = 7147.7  # Total nm for port-to-working-area transits
        
        assert abs(stats["port_area"]["total_duration_h"] - expected_hours) < 1.0, \
            f"Expected ~{expected_hours}h working area transits, got {stats['port_area']['total_duration_h']:.1f}h"
        
        assert abs(stats["port_area"]["total_distance_nm"] - expected_distance) < 10.0, \
        ...
        assert 9.0 < stats["port_area"]["avg_speed_kts"] < 12.0, \
    ...
    def test_tc3_complete_workflow(self, tc3_config):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            assert len(timeline) > 50, f"Expected >50 activities for 6-leg cruise, got {len(timeline)}"