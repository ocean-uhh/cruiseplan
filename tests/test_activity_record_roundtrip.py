...
def test_activity_record_netcdf_roundtrip():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        netcdf_files = [f for f in result.files_created if f.suffix == '.nc' and 'schedule' in f.name]
        assert len(netcdf_files) == 1, f"Expected 1 schedule NetCDF file, got {len(netcdf_files)}"
        ...
        original_records = [ActivityRecord(activity_data) for activity_data in original_timeline]
        
        # Debug: print activity types to understand the data
        print(f"\nActivity types in timeline:")
        for i, activity in enumerate(original_timeline):
            print(f"  {i}: {activity['label']} -> activity='{activity['activity']}', category='{activity.get('category', 'N/A')}'")
        ...
        for i, (orig, recovered) in enumerate(zip(original_records, recovered_records)):
            ...
            print(f"  Original: activity='{orig.activity}', recovered: activity='{recovered.activity}'")
            
            # Activity should be preserved exactly now
            assert recovered.activity == orig.activity, f"Activity mismatch: {recovered.activity} != {orig.activity}"
            assert recovered.label == orig.label, f"Label mismatch: {recovered.label} != {orig.label}"
            assert recovered.leg_name == orig.leg_name, f"Leg name mismatch: {recovered.leg_name} != {orig.leg_name}"
            
            # Operation type and class should be preserved exactly now
            print(f"op_type: orig='{orig.op_type}', recovered='{recovered.op_type}'")
            print(f"operation_class: orig='{orig.operation_class}', recovered='{recovered.operation_class}'")
            
            assert recovered.op_type == orig.op_type, f"Op type mismatch: {recovered.op_type} != {orig.op_type}"
            assert recovered.operation_class == orig.operation_class, f"Operation class mismatch: {recovered.operation_class} != {orig.operation_class}"
            
            # Coordinate fields (with small floating point tolerance)
            assert abs(recovered.entry_lat - orig.entry_lat) < 1e-6, f"Entry lat mismatch: {recovered.entry_lat} != {orig.entry_lat}"
            assert abs(recovered.entry_lon - orig.entry_lon) < 1e-6, f"Entry lon mismatch: {recovered.entry_lon} != {orig.entry_lon}"
            assert abs(recovered.exit_lat - orig.exit_lat) < 1e-6, f"Exit lat mismatch: {recovered.exit_lat} != {orig.exit_lat}"
            assert abs(recovered.exit_lon - orig.exit_lon) < 1e-6, f"Exit lon mismatch: {recovered.exit_lon} != {orig.exit_lon}"
            ...
            if orig.action is not None:
                ...
                assert recovered.action == orig.action, f"Action mismatch: {recovered.action} != {orig.action}"
                
            if orig.operation_depth is not None:
                assert recovered.operation_depth is not None, "Operation depth should not be None"
                ...
                assert recovered.operation_depth is None or np.isnan(recovered.operation_depth), "Operation depth should be None or NaN"
                
            if orig.water_depth is not None:
                assert recovered.water_depth is not None, "Water depth should not be None"
                assert abs(recovered.water_depth - orig.water_depth) < 0.1, \
                    f"Water depth mismatch: {recovered.water_depth} != {orig.water_depth}"
            else:
                assert recovered.water_depth is None or np.isnan(recovered.water_depth), "Water depth should be None or NaN"
        
        print(f"\n✅ Successfully verified complete roundtrip preservation of {len(recovered_records)} ActivityRecord objects")
        print(f"✅ All ActivityRecord fields properly preserved through real cruise schedule generation!")
...
def test_activity_record_required_fields_present():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        assert not missing_vars, f"Missing ActivityRecord variables in NetCDF: {missing_vars}"
        
        print(f"✅ All {len(expected_variables)} expected ActivityRecord variables present in NetCDF")
