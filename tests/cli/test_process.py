...
from cruiseplan.cli.process import main, determine_steps
...
class TestProcessFlagValidation:
    ...
    def test_multiple_only_flags_error(self):
        ...
        with pytest.raises(CLIError, match="Cannot specify multiple --only-\\* flags together"):
    ...
    def test_conflicting_only_and_no_flags_error(self):
        ...
        with pytest.raises(CLIError, match="Cannot specify both --only-enrich and --no-enrich"):
            args = self.create_args(only_enrich=True, no_enrich=True)
            determine_steps(args)
            
        with pytest.raises(CLIError, match="Cannot specify both --only-validate and --no-validate"):
            args = self.create_args(only_validate=True, no_validate=True)
            determine_steps(args)
            
        with pytest.raises(CLIError, match="Cannot specify both --only-map and --no-map"):
    ...
    def test_only_enrich_with_format_error(self):
        ...
        with pytest.raises(CLIError, match="Cannot specify --only-enrich with --format"):
    ...
    def test_only_validate_with_format_error(self):
        ...
        with pytest.raises(CLIError, match="Cannot specify --only-validate with --format"):
    ...
    def test_all_steps_disabled_error(self):
        ...
        with pytest.raises(CLIError, match="All processing steps disabled - nothing to do"):
...
class TestProcessIntegration:
    ...
    def test_only_enrich_calls_enrich_main(self, mock_enrich, mock_setup_logging, mock_validate_output, mock_validate_input):
    ...
    def test_only_validate_calls_validate_main(self, mock_validate, mock_setup_logging, mock_validate_output, mock_validate_input):
    ...
    def test_only_map_calls_map_main(self, mock_map, mock_setup_logging, mock_validate_output, mock_validate_input):
    ...
    @patch('cruiseplan.cli.validate.main')
    @patch('cruiseplan.cli.enrich.main')
    def test_full_processing_pipeline(self, mock_enrich, mock_validate, mock_map, 
                                    mock_setup_logging, mock_validate_output, mock_validate_input):
    ...
    @patch('cruiseplan.cli.validate.main')
    @patch('cruiseplan.cli.enrich.main')
    def test_selective_processing_no_map(self, mock_enrich, mock_validate,
                                       mock_setup_logging, mock_validate_output, mock_validate_input):
    ...
    def test_enrichment_smart_defaults(self, mock_setup_logging, mock_validate_output, mock_validate_input):
    ...
    def test_legacy_parameter_warnings(self, mock_setup_logging, mock_validate_output, mock_validate_input):
...
class TestProcessEdgeCases:
    ...
    def create_args(self, **kwargs):
        ...
        for attr in ['only_enrich', 'only_validate', 'only_map', 'no_enrich', 'no_validate', 'no_map',
                     'no_depths', 'no_coords', 'no_sections', 'no_ports', 'no_depth_check', 'strict',
                     'verbose', 'quiet']:
    ...
    @patch('cruiseplan.cli.process.setup_logging')
    @patch('cruiseplan.cli.map.main')
    def test_map_generation_failure(self, mock_map, mock_setup_logging, mock_validate_output, mock_validate_input):
