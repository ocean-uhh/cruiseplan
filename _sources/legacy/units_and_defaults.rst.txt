...
- **DDM (Degrees, Decimal Minutes)**: ``75°35.000'N, 015°15.000'W`` (enrichment output)
...
.. code-block:: yaml
    ...
    # After enrichment (DDM format added)
    coordinates_ddm: "60 30.00'N, 020 45.00'W"
...
.. list-table:: **Operation Defaults**
   ...
   * - ``default_mooring_duration``
     - 59940.0 minutes
     - Default mooring duration (999 hours - user must override)
   * - ``day_start_hour``
     - 8
     - Start hour for daylight operations window
   * - ``day_end_hour``
     - 20
     - End hour for daylight operations window
...
CruisePlan uses centralized helper functions in `cruiseplan/config/values.py` instead of magic numbers:

.. code-block:: python

   # From cruiseplan/config/values.py
...
.. code-block:: python
   ...
   from cruiseplan.config.values import DEFAULT_VESSEL_SPEED_KT, DEFAULT_TURNAROUND_TIME_MIN
