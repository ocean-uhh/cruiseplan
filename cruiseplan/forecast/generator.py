...
def list_activities(schedule: xr.Dataset) -> List[Tuple[int, str, str, str, float, str, float, float, float]]:
    """
    ...
    if hasattr(time_data[0], 'total_seconds'):
        ...
        time_offsets = [pd.Timedelta(seconds=float(t.total_seconds())) for t in time_data]
    ...
    for i, (time_offset, category, duration, name) in enumerate(zip(
        time_offsets,
        schedule.type.values if 'type' in schedule.variables else schedule.category.values,
        ...
        activities.append((i, time_str, category_str, duration_str, duration_hours, name_str, water_depth, operation_depth, distance))
    
    return activities


def format_activities_table(activities: List[Tuple[int, str, str, str, float, str, float, float, float]]) -> str:
    """
    ...
    # Table rows
    for idx, time_offset, category, duration_str, _, name, water_depth, operation_depth, distance in activities:
        # Format depth columns (show if available, including 0)
        water_depth_str = f"{water_depth:8.0f}m" if water_depth is not None else "        -"
        op_depth_str = f"{operation_depth:6.0f}m" if operation_depth is not None else "      -"
...
def generate_forecast(
    ...
    if start_index < 0 or start_index >= num_activities:
        raise ValueError(f"start_index {start_index} out of range (0-{num_activities-1})")
    ...
    for i in range(start_index, num_activities):
        ...
        activity_type = str(schedule.type[i].values).strip() if 'type' in schedule.variables else ''
        action = str(schedule.action[i].values).strip() if 'action' in schedule.variables else ''
        ...
        forecast_activities.append((
            i, absolute_time.to_pydatetime(), category, activity_type, action, 
            duration, latitude, longitude, name
        ))
    
    logger.info(f"Generated forecast with {len(forecast_activities)} activities "
