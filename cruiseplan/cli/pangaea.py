...
def fetch_pangaea_data(doi_list: List[str], 
                      rate_limit: float = 1.0,
                      merge_campaigns: bool = True) -> List[dict]:
    """
...
def save_pangaea_pickle(datasets: List[dict], output_path: Path, original_count: int = None) -> None:
    """
...
def main(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if not datasets:
            logger.warning("No datasets retrieved. Check DOI list and network connection.")
            return
