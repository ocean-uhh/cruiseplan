...
def run_enrichment(args: argparse.Namespace, config_file: Path, output_path: Optional[Path] = None) -> Path:
    """
    ...
    add_depths = not getattr(args, 'no_depths', False)  # Default enabled unless --no-depths
    add_coords = not getattr(args, 'no_coords', False)  # Default enabled unless --no-coords
    expand_sections = not getattr(args, 'no_sections', False)  # Default enabled unless --no-sections
    expand_ports = not getattr(args, 'no_ports', False)  # Default enabled unless --no-ports
    ...
    if summary.get("sections_expanded", 0) > 0:
        logger.info(f"✓ Expanded {summary['sections_expanded']} CTD sections into {summary.get('stations_from_expansion', 0)} stations")
...
def run_validation(args: argparse.Namespace, config_file: Path) -> bool:
    ...
    check_depths = not getattr(args, 'no_depth_check', False)  # Default enabled unless --no-depth-check
...
def main(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if hasattr(args, 'bathy_dir_legacy') and args.bathy_dir_legacy:
            logger.warning("⚠️  WARNING: '--bathymetry-dir' is deprecated. Use '--bathy-dir' instead.")
            args.bathy_dir = args.bathy_dir_legacy
        
        if hasattr(args, 'bathy_stride_legacy') and args.bathy_stride_legacy:
            logger.warning("⚠️  WARNING: '--bathymetry-stride' is deprecated. Use '--bathy-stride' instead.")
            args.bathy_stride = args.bathy_stride_legacy
        
        if hasattr(args, 'coord_format_legacy') and args.coord_format_legacy:
            logger.warning("⚠️  WARNING: '--coord-format' is deprecated. Coordinate format is now fixed to DMM.")
        ...
        if no_map and map_formats:
            raise CLIError("Cannot specify --no-map with --format (map formats requested but mapping disabled)")
        
        if only_enrich and map_formats:
            raise CLIError("Cannot specify --only-enrich with --format (enrichment mode doesn't generate maps)")
        
        if only_validate and map_formats:
            raise CLIError("Cannot specify --only-validate with --format (validation mode doesn't generate maps)")
        ...
        if hasattr(args, 'figsize') and args.figsize != [12, 8]:  # Non-default figsize
            if not map_formats or "png" not in map_formats:
                logger.warning("⚠️  --figsize specified but PNG format not requested - figsize will be ignored")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no-depths", action="store_true", 
    ...
    parser.add_argument("--bathy-stride", type=int, default=10,
                       help="Bathymetry contour stride (default: 10)")
    
    # Legacy argument support with deprecation warnings
    parser.add_argument("--bathymetry-source", dest="bathy_source_legacy", 
                       choices=["etopo2022", "gebco2025"],
                       help="[DEPRECATED] Use --bathy-source instead")
    parser.add_argument("--bathymetry-dir", type=Path, dest="bathy_dir_legacy",
                       help="[DEPRECATED] Use --bathy-dir instead")
    parser.add_argument("--bathymetry-stride", type=int, dest="bathy_stride_legacy",
                       help="[DEPRECATED] Use --bathy-stride instead")
    parser.add_argument("--coord-format", dest="coord_format_legacy", choices=["dmm", "dms"],
