...
from cruiseplan.utils.activity_utils import round_time_to_minute, format_operation_action
...
class CSVGenerator:
    ...
    def generate_schedule_csv(self, config: CruiseConfig, timeline: List[ActivityRecord], output_file: Path) -> Path:
        """
        ...
        fieldnames = [
            ...
            'Transit dist [nm]', 'Vessel speed [kt]', 'Duration [hrs]', 'Depth [m]',
            'Lat [deg]', 'Lon [deg]', 'Lat [deg_rounded]', 'Lat [min]',
        ...
        with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            for activity in timeline:
                ...
                lat_deg_float, lat_min = UnitConverter.decimal_degrees_to_dmm(lat_decimal)
                lon_deg_float, lon_min = UnitConverter.decimal_degrees_to_dmm(lon_decimal)
                ...
                row = {
                    ...
                    'Lat [deg]': round(lat_decimal, 6),  # High precision decimal degrees
                    'Lon [deg]': round(lon_decimal, 6),  # High precision decimal degrees
...
def generate_csv_schedule(config: CruiseConfig, timeline: List[ActivityRecord], output_file: Path) -> Path:
    """
