...
from cruiseplan.utils.activity_utils import is_scientific_operation, is_line_operation
...
class KMLGenerator:
    ...
    def generate_schedule_kml(self, config: CruiseConfig, timeline: List[ActivityRecord], output_file: Path) -> Path:
        """
...
'''
        ...
        scientific_activities = [activity for activity in timeline if is_scientific_operation(activity)]
...
'''
                ...
                style_id = "stationStyle" if activity['activity'] == "Station" else "mooringStyle"
                depth_str = f"Depth: {activity.get('depth', 0):.1f} m" if activity.get('depth') else ""
...
'''
        ...
        logger.info(f"KML schedule saved to: {output_file} ({len(scientific_activities)} scientific operations)")
        return output_file


def generate_kml_schedule(config: CruiseConfig, timeline: List[ActivityRecord], output_file: Path) -> Path:
    """
