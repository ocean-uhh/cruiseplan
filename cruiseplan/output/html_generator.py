...
def _calculate_summary_statistics(timeline):
    ...
    scientific_transits = [a for a in all_transits if is_scientific_operation(a) and a["activity"] == "Transit"]
    ...
    if mooring_activities:
        total_mooring_duration_h = sum(a["duration_minutes"] for a in mooring_activities) / 60
        ...
        mooring_stats = {"count": 0, "avg_duration_h": 0, "total_duration_h": 0, "total_duration_days": 0}
    
    # Calculate station statistics  
    station_stats = {}
    if station_activities:
        total_station_duration_h = sum(a["duration_minutes"] for a in station_activities) / 60
        avg_station_duration_h = total_station_duration_h / len(station_activities)
        avg_depth = sum(a.get("depth", 0) for a in station_activities) / len(station_activities)
        station_stats = {
        ...
        station_stats = {"count": 0, "avg_duration_h": 0, "avg_depth_m": 0, "total_duration_h": 0, "total_duration_days": 0}
    
    # Calculate survey operations (scientific transits)
    survey_stats = {}
    if scientific_transits:
        total_survey_duration_h = sum(a["duration_minutes"] for a in scientific_transits) / 60
        avg_survey_duration_h = total_survey_duration_h / len(scientific_transits)
        total_survey_distance_nm = sum(a.get("operation_dist_nm", 0) for a in scientific_transits)
        ...
        survey_stats = {"count": 0, "avg_duration_h": 0, "avg_distance_nm": 0, "total_duration_h": 0, "total_duration_days": 0}
    
    # Calculate within-area navigation transits (excluding first and last)
    within_area_transits = navigation_transits[1:-1] if len(navigation_transits) > 2 else []
    within_area_stats = {}
    if within_area_transits:
        total_within_duration_h = sum(a["duration_minutes"] for a in within_area_transits) / 60
        total_within_distance_nm = sum(a.get("transit_dist_nm", 0) for a in within_area_transits)
        avg_speed_kts = total_within_distance_nm / total_within_duration_h if total_within_duration_h > 0 else 0
        within_area_stats = {
        ...
        within_area_stats = {"total_distance_nm": 0, "avg_speed_kts": 0, "total_duration_h": 0, "total_duration_days": 0}
    ...
    if port_transits:
        ...
        avg_speed_kts = total_port_distance_nm / total_port_duration_h if total_port_duration_h > 0 else 0
        port_area_stats = {
        ...
        port_area_stats = {"total_distance_nm": 0, "avg_speed_kts": 0, "total_duration_h": 0, "total_duration_days": 0}
...
class HTMLGenerator:
    ...
    def generate_schedule_report(self, config: CruiseConfig, timeline: List[ActivityRecord], output_file: Path) -> Path:
        """
        ...
        total_duration_h = (stats["moorings"]["total_duration_h"] + 
...
def generate_html_schedule(config: CruiseConfig, timeline: List[ActivityRecord], output_file: Path) -> Path:
    """
