...
from cruiseplan.forecast.generator import list_activities, format_activities_table, generate_forecast
    ...
    try:
        ...
        if not activities:
            return StationplanResult(
                success=False,
                message="No activities found in schedule file",
                output=""
            )
...
def stationplan_forecast(
    ...
    """
    try:
        logger.info(f"Generating forecast from {schedule_file} starting at index {start_index}")
        ...
        output_text = format_letsgo_output(forecast_activities, start_time, transit_speed)
        
        logger.info(f"Successfully generated forecast with {len(forecast_activities)} activities")
...
def stationplan_forecast_tex(
    ...
    try:
        ...
        schedule_path = Path(schedule_file)
        logger.info(f"Generating TeX forecast from {schedule_path} starting at index {start_index}")
        ...
        for activity_tuple in forecast_activities:
            ...
            try:
                if 'exit_latitude' in schedule.variables and 'exit_longitude' in schedule.variables:
...
def stationplan_waypoints(
    ...
    try:
        from cruiseplan.forecast.reader import read_schedule
        from cruiseplan.forecast.generator import list_activities, generate_forecast
        ...
        if start_time is None:
            ...
            start_time = now.replace(hour=hours % 24, minute=minutes, second=0, microsecond=0).isoformat()
        
        # Always use forecast mode for waypoints (from start_index forward)
        activities = generate_forecast(schedule, start_index, start_time, duration_hours or 1000)
        
        if not activities:
            return StationplanResult(
                success=False,
                message="No activities found in schedule",
                output=""
            )
        ...
        # Add legend header  
        waypoint_lines.append("% (1: Transit, 2: CTD, 3: Mooring, 4: PIES, 5: Float/Drifter, 6: Survey)")
        waypoint_lines.append("%")
        ...
        for activity in activities:
            if len(activity) >= 9:
                index, time, category, activity_type, action, duration, latitude, longitude, name = activity
                ...
                if index < len(schedule.latitude) - 1:
                    ...
                    try:
                        if 'exit_latitude' in schedule.variables and 'exit_longitude' in schedule.variables:
                            ...
                            if ('Transit_' in name or 'Bathy_' in name or 
                                activity_type in ['survey', 'unknown', 'underway']):
                ...
                if abs(latitude - exit_lat) > 0.001 or abs(longitude - exit_lon) > 0.001:
                    ...
                    waypoint_lines.append(f"{work_code}, {entry_lat_ddm}, {entry_lon_ddm}, {name} Start")
                    waypoint_lines.append(f"{work_code}, {exit_lat_ddm}, {exit_lon_ddm}, {name} End")
                else:
                    # Same position - show single waypoint without suffix
                    waypoint_lines.append(f"{work_code}, {entry_lat_ddm}, {entry_lon_ddm}, {name}")
...
def _map_activity_to_work_code(category: str, activity_type: str, name: str = "") -> int:
    """
