...
def _format_timeline_summary(timeline: List[Dict], total_duration: float) -> str:
    ...
    if activity_types:
        type_summary = ", ".join([f"{count} {op_type}" for op_type, count in activity_types.items()])
...
def _format_coordinate_summary(lat_range: Tuple[float, float], lon_range: Tuple[float, float]) -> str:
    """
...
def _format_validation_results(success: bool, errors: List[str], warnings: List[str]) -> str:
    """
