...
def _validate_directory_writable(dir_path: Path, create_if_missing: bool = True) -> Path:
    """
...
def _validate_coordinate_bounds(lat_bounds: List[float], lon_bounds: List[float]) -> Tuple[float, float, float, float]:
    """
    ...
    if not isinstance(lat_bounds, list) or len(lat_bounds) != 2:
        raise ValueError("lat_bounds must be a list of exactly 2 values [min_lat, max_lat]")
        
    if not isinstance(lon_bounds, list) or len(lon_bounds) != 2:
        raise ValueError("lon_bounds must be a list of exactly 2 values [min_lon, max_lon]")
    ...
    if not (-90 <= min_lat <= 90):
        raise ValueError(f"Invalid minimum latitude: {min_lat}. Must be between -90 and 90.")
        
    if not (-90 <= max_lat <= 90):
        raise ValueError(f"Invalid maximum latitude: {max_lat}. Must be between -90 and 90.")
        
    if not (-180 <= min_lon <= 180):
        raise ValueError(f"Invalid minimum longitude: {min_lon}. Must be between -180 and 180.")
        
    if not (-180 <= max_lon <= 180):
        raise ValueError(f"Invalid maximum longitude: {max_lon}. Must be between -180 and 180.")
...
def _validate_numeric_range(value: float, min_val: float, max_val: float, name: str) -> float:
    """
...
def _validate_format_list(formats: List[str], valid_formats: List[str]) -> List[str]:
    ...
    if invalid_formats:
        raise ValueError(
            f"Invalid formats: {invalid_formats}. "
...
def _detect_pangaea_mode(args) -> Tuple[str, dict]:
    ...
    if Path(query_or_file).exists() and Path(query_or_file).is_file():
        ...
        if not _validate_file_extension(Path(query_or_file), ['.txt', '.doi']):
            logger.warning(f"File '{query_or_file}' doesn't have expected extension (.txt or .doi)")
